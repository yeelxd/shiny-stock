...
class StockStat(object):
    ...
    # 获取股票列表
    def get_stock_list(self, stock_list_url, stock_info_url):
        html = self.get_html_text(stock_list_url)
        # Page Html
        ...
        for a in all_a:
            try:
                ...
                if len(stock_code) > 0:
                    ...
                    # 谨防重复处理
                    if self.STOCK_URL_LIST.count(stock_url) == 0:
                        self.STOCK_URL_LIST.append(stock_url)
            except Exception as e:
                print("获取股票列表Err.", e)
                continue

    # 获取股票详情信息
    def package_stock_info(self, stock_info_url):
        # Debugger URL
        ...
        html = self.get_html_text(stock_info_url)
        # Http出错再获取一次
        ...
        try:
            ...
            attention_html = self.get_html_text(attention_url)
            if attention_html != "":
            ...
            # 封装好的信息添加到待处理列表
            self.STOCK_INFO_LIST.append(stock_info)
            print("获取成功: pid={}, url={}".format(os.getpid(), stock_info_url))
            print("所在列表位置: {}/{}".format(self.STOCK_URL_LIST.index(stock_info_url),
                                         len(self.STOCK_URL_LIST)))
        except Exception as e:
    ...
    def main_center(self):
        ...
        self.get_stock_list(stock_list_url, stock_info_url)
        # 筛选股票信息保存到数据库中
        print("待筛选的股票数量={}".format(len(self.STOCK_URL_LIST)))
        # 记录开始时间
        start_time = time.time()
        # 使用四个进程处理任务
        pool = Pool(4)
        pool.map(self.package_stock_info, self.STOCK_URL_LIST)
        pool.close()
        pool.join()
        # 列表去重
        new_stock_info_list = []
        ...
        # 首先删除当天的记录数据
        mysql_util_instance.remove_by_trade_date(time.strftime("%Y-%m-%d", time.localtime()))
